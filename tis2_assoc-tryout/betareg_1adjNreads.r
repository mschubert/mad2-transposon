...
do_fit = function(data) {
    ...
    mod = betareg(aneup ~ reads_smp + reads, data=data2)
    mod$model = na.omit(select(data2, sample, aneup, reads, reads_smp))
    mod %>%
        broom::tidy() %>%
        filter(term == "reads") %>%
...
assoc = . %>%
    group_by(sample, gene_name, ensembl_gene_id, aneup, reads_smp) %>%
