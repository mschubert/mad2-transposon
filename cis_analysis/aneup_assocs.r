...
do_test = function(dset, field) {
    dset %>%
        group_by(external_gene_name) %>%
        tidyr::nest() %>%
        mutate(fit = purrr::map(data, poisson_reg, field=field)) %>%
        select(-data) %>%
        tidyr::unnest() %>%
...
sys$run({
    args = sys$cmd$parse(
        ...
        opt('o', 'outfile', 'aneuploidy assocs', 'aneup_assocs/gene.RData'),
        opt('p', 'plotfile', 'pdf', 'aneup_assocs/gene.pdf'))
    ...
    results = sapply(fields, do_test, dset=aset, simplify=FALSE)
