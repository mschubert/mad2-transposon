...
args = sys$cmd$parse(
    opt('e', 'eset', 'gene expression rds', 'eset_Mad2PB+MycV2.rds'),
    opt('c', 'config', 'yaml', '../config.yaml'),
    opt('n', 'network', 'rds', '../data/networks/E-GEOD-13159.rds'), # ignored
    opt('o', 'outfile', 'results rds', 'de_Mad2PB+MycV2.rds'),
    opt('p', 'plotfile', 'pdf', 'de_Mad2PB+MycV2.pdf'),
    arg('sets', 'gene set .rds', arity='*',
        list.files("../data/genesets/mouse", "\\.rds", full.names=TRUE))
)

gset = readRDS(grep("HALLMARK", args$sets, value=TRUE))$HALLMARK_MYC_TARGETS_V2
eset = readRDS(args$eset)$eset[gset,]
