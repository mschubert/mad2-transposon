...
args = sys$cmd$parse(
    opt('e', 'eset', 'gene expression RData', 'eset_Mad2PB+MycV2.RData'),
    opt('c', 'config', 'yaml', '../config.yaml'),
    opt('n', 'network', 'RData', '../data/networks/E-GEOD-13159.RData'), # ignored
    opt('o', 'outfile', 'results RData', 'de_Mad2PB+MycV2.RData'),
    opt('p', 'plotfile', 'pdf', 'de_Mad2PB+MycV2.pdf'),
    arg('sets', 'gene set .RData', arity='*',
        list.files("../data/genesets/mouse", "\\.RData", full.names=TRUE)))

gset = io$load(grep("HALLMARK", args$sets, value=TRUE))$HALLMARK_MYC_TARGETS_V2
eset = io$load(args$eset)$eset[gset,]
