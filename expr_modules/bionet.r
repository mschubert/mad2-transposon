...
#TODO: use shape on real DE data to indicate whether ins present (discard here)
shape = rep(1, length(nodes)) #stats$statistic[match(nodes, toupper(stats$expr))]
shape[is.na(shape)] = -1
names(shape) = nodes(interactome)

module = runFastHeinz(interactome, scores)
plotModule(module, scores=shape, diff.expr=diff_expr)
dev.off()
